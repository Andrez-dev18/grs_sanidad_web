...
function upload_error_message($code) {
    switch ((int)$code) {
        ...
        case UPLOAD_ERR_INI_SIZE: return 'UPLOAD_ERR_INI_SIZE (excede upload_max_filesize)';
        case UPLOAD_ERR_FORM_SIZE: return 'UPLOAD_ERR_FORM_SIZE (excede MAX_FILE_SIZE del formulario)';
        case UPLOAD_ERR_PARTIAL: return 'UPLOAD_ERR_PARTIAL (subida parcial)';
        case UPLOAD_ERR_NO_FILE: return 'UPLOAD_ERR_NO_FILE (no se subió archivo)';
        case UPLOAD_ERR_NO_TMP_DIR: return 'UPLOAD_ERR_NO_TMP_DIR (falta tmp dir)';
        case UPLOAD_ERR_CANT_WRITE: return 'UPLOAD_ERR_CANT_WRITE (no se pudo escribir en disco)';
        case UPLOAD_ERR_EXTENSION: return 'UPLOAD_ERR_EXTENSION (bloqueado por extensión)';
        default: return 'UNKNOWN_UPLOAD_ERROR';
    }
}
...
foreach ($items as $itemIndex => $item) {
    ...
    $tdate = $item['tdate'] ?? date('Y-m-d');
    $ttime = $item['ttime'] ?? date('H:i:s');
    $diareg = $item['diareg'] ?? $tdate;
    ...
    if (!empty($_FILES)) {
        error_log("=== ARCHIVOS RECIBIDOS EN \$_FILES (GUARDAR) ===");
        foreach ($_FILES as $key => $fileInfo) {
            ...
            $errMsg = is_numeric($errorCode) ? upload_error_message((int)$errorCode) : 'N/A';
            error_log("  - Key: '$key' | Name: '$name' | Size: $size bytes | Error: $errorCode ($errMsg)");
            $imagenesRecibidas++;
        }
        error_log("=== FIN ARCHIVOS (GUARDAR) ===");
    }
        ...
        for ($i = 0; $i < $cantidad; $i++) {
            ...
            if ($fileFound && isset($_FILES[$actualFileKey]) && $_FILES[$actualFileKey]['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($file['tmp_name'], $rutaFisica)) {
                    ...
                    $imagenesErrores[] = "No se pudo mover archivo ($actualFileKey) -> $nombreArchivo";
                }
            } else {
                $errorCode = isset($_FILES[$actualFileKey]) ? $_FILES[$actualFileKey]['error'] : 'NO_EXISTE';
                $errMsg = is_numeric($errorCode) ? upload_error_message((int)$errorCode) : 'NO_EXISTE';
                $imagenesErrores[] = "Archivo no encontrado o con error ($fileKey): $errorCode ($errMsg)";
            }
        }
        if (!empty($rutasNivel)) {
...
echo json_encode([
    ...
    "message" => $totalProcesados > 0 
        ? ("Datos procesados correctamente. Insertados: $insertados, Duplicados: $duplicados" .
           ($imagenesRecibidas > 0 && $imagenesGuardadas == 0 ? " (Advertencia: no se pudo guardar ninguna imagen; revise límites del servidor)" : ""))
        : "No se procesaron registros. Verifique los logs para más detalles.",
    "detalle" => array_merge($respuesta, [
        "imagenes_recibidas" => $imagenesRecibidas,
        "imagenes_guardadas" => $imagenesGuardadas,
        "imagenes_errores" => $imagenesErrores,
        "php_limits" => $phpUploadLimits
    ])
], JSON_UNESCAPED_UNICODE);
?>
