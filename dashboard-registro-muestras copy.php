...
<body class="bg-gray-50">
    <div class="container mx-auto px-6 py-12">
                    ...
                    <div class="form-section mb-8">
                        ...
                        <div class="form-field">
                            ...
                            <input type="number" id="numeroSolicitudes" name="numeroSolicitudes" min="1" max="20"
                                placeholder="Ingrese cantidad de solicitudes"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                        </div>
                    </div>
      ...
      Cancelar
        ...
        <div id="confirmModal" class="fixed inset-0 z-50 hidden" role="dialog" aria-modal="true">
            <div class="absolute inset-0 bg-gray-900 bg-opacity-75 transition-opacity" onclick="closeConfirmModal()">
            </div>

            <div class="fixed inset-0 z-10 w-screen overflow-hidden p-4 sm:p-6 flex items-center justify-center">
                <div
                    class="relative flex flex-col w-full max-w-6xl max-h-[90vh] bg-white rounded-xl shadow-2xl overflow-hidden">

                    <!-- HEADER -->
                    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-white shrink-0">
                        <h3 class="text-xl font-bold text-gray-800">
                            📋 Confirmar Envío de Muestras
                        </h3>
                        <button type="button"
                            class="text-gray-400 hover:text-red-500 transition-colors text-3xl leading-none"
                    ...
                    <div class="flex-1 min-h-0 overflow-y-auto p-6 bg-gray-50">
                        <div id="summaryContent" class="w-full">
                    ...
                    <!-- FOOTER -->
                    <div
                        class="px-6 py-4 border-t border-gray-200 bg-white flex flex-col sm:flex-row justify-end gap-3 shrink-0">
                        <button type="button" onclick="closeConfirmModal()"
                            class="px-5 py-2.5 rounded-lg bg-white border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 transition-colors">
                            Cancelar
                        </button>
                        <button type="button" onclick="confirmSubmit()"
                            class="px-5 py-2.5 rounded-lg bg-gradient-to-r from-blue-600 to-blue-700 text-white font-medium hover:from-blue-700 hover:to-blue-800 shadow-md transition-all flex items-center gap-2">
    ...
    <script>
        ...
        async function cargarTiposMuestra(selectId, sampleIndex) {
            try {
                ...
                // ✅ SOLUCIÓN: Remover listener anterior si existe
                ...
                select._changeHandler = function () {
                    const sampleIndex = parseInt(this.dataset.sampleIndex);
                    sampleDataCache[sampleIndex] = sampleDataCache[sampleIndex] || {};
                    sampleDataCache[sampleIndex].tipoMuestra = this.value;
                    sampleDataCache[sampleIndex].analisisSeleccionados = [];
                    document.getElementById(`analisisResumen_${sampleIndex}`).innerHTML = 'Ninguno';
                    updateCodigoReferencia(sampleIndex);
                };

                // Asignar el índice y agregar el listener
        ...
        // ✅ Ya no necesitas la función handleTipoMuestraChange separada
        ...
        /*function handleTipoMuestraChange(event) {
            const select = event.target;
            const sampleIndex = parseInt(select.dataset.sampleIndex);
            // ✅ Guardar tipo en caché
            sampleDataCache[sampleIndex] = sampleDataCache[sampleIndex] || {};
            sampleDataCache[sampleIndex].tipoMuestra = select.value;
            // ✅ Limpiar análisis (como pediste)
            sampleDataCache[sampleIndex].analisisSeleccionados = [];
            document.getElementById(`analisisResumen_${sampleIndex}`).innerHTML = 'Ninguno';
            ...
            for (let i = 0; i < count; i++) {
                ...
                cargarTiposMuestra(`tipoMuestra_${i}`, i).then(() => {
                    ...
                    if (cache && cache.tipoMuestra) {
                        ...
                        // ✅ Actualizar código de referencia y luego restaurar campos
                        updateCodigoReferencia(i).then(() => {
                            // Ahora sí existe #codigoReferenciaBoxes_i
                            ...
                            console.warn("No se pudo restaurar el código de referencia:", err);
                            // Aún así restaurar lo demás
                            if (cache.fechaToma) document.getElementById(`fechaToma_${i}`).value = cache.fechaToma;
                            // ... etc
                        });
                    }
        ...
        function generateTableRows(count) {
            ...
            // Crear las filas en un fragmento de documento
            ...
            for (let i = 0; i < count; i++) {
                ...
                // 1. Tipo de Muestra - SIMPLIFICADO
                const tmCell = document.createElement('td');
                tmCell.innerHTML = `
            <select id="tipoMuestra_${i}" name="tipoMuestra_${i}" 
                    class="form-select tipo-muestra-select" 
                    data-sample-index="${i}">
                <option value="">Seleccionar...</option>
            </select>
        `;
            ...
            <div class="d-flex flex-column">
                <div class="d-flex gap-1 mb-1">
                    <button type="button" class="btn btn-sm btn-outline-primary" onclick="openAnalisisModal(${i})">Seleccionar</button>
                    <button type="button" class="btn btn-sm btn-outline-secondary" onclick="copyAnalisisTo(${i})">Copiar</button>
                </div>
                <div id="analisisResumen_${i}" style="font-size: 0.85em; min-height: 1.2em;">Ninguno</div>
            </div>`;
                row.appendChild(anCell);

                // 6. Observaciones
                const obsCell = document.createElement('td');
                obsCell.innerHTML = `<textarea class="form-control" id="observaciones_${i}" rows="2" placeholder="Observaciones..."></textarea>`;
                row.appendChild(obsCell);
            ...
            // Ahora llenar los selects y restaurar datos
            setTimeout(() => {
                for (let i = 0; i < count; i++) {
                    const select = document.getElementById(`tipoMuestra_${i}`);
                    if (select) {
                        // Llenar opciones
                        select.innerHTML = optionsMuestraHTML;

                        // Restaurar valor si existe
                        const cache = sampleDataCache[i];
                        if (cache && cache.tipoMuestra) {
                            select.value = cache.tipoMuestra;
                        ...
                        select.addEventListener('change', function () {
                            const idx = parseInt(this.dataset.sampleIndex);
                            sampleDataCache[idx] = sampleDataCache[idx] || {};
                            sampleDataCache[idx].tipoMuestra = this.value;
                        ...
                        // Forzar reflow para que Bootstrap lo reconozca
                    ...
                    if (cache) {
                        ...
                        if (cache.numeroMuestras) document.getElementById(`numeroMuestras_${i}`).value = cache.numeroMuestras;
                        if (cache.observaciones) document.getElementById(`observaciones_${i}`).value = cache.observaciones;
                        if (cache.analisisSeleccionados?.length > 0) {
                            updateAnalisisResumen(i);
                        }
                    }
                }
        ...
        function copyAnalisisTo(sourceIndex) {
            ...
            // Crear contenedor temporal para el select
            const selectContainer = document.createElement('div');
            selectContainer.innerHTML = `
        <div style="padding:10px; background:#f9f9f9; border:1px solid #ccc; border-radius:6px;">
            <label><strong>Copiar a la fila:</strong></label>
            <select id="targetRowSelect" class="form-select mt-2">
                <option value="">Seleccionar...</option>
            </select>
            <div class="mt-2">
                <button id="btnCopyConfirm" class="btn btn-sm btn-primary">Copiar</button>
                <button id="btnCopyCancel" class="btn btn-sm btn-secondary ms-2">Cancelar</button>
            </div>
        </div>
    `;

            const analisisResumenCell = document.querySelector(`#analisisResumen_${sourceIndex}`).closest('td');
            analisisResumenCell.appendChild(selectContainer);

            const selectEl = document.getElementById('targetRowSelect');
            const confirmBtn = document.getElementById('btnCopyConfirm');
            const cancelBtn = document.getElementById('btnCopyCancel');

            // Llenar opciones (solo filas válidas ≠ sourceIndex)
            for (let i = 0; i < totalSamples; i++) {
                if (i !== sourceIndex) {
            ...
            confirmBtn.onclick = () => {
                ...
                // Solo cambiar tipo si es diferente
                ...
                document.getElementById(`analisisResumen_${targetIndex}`).innerHTML =
                    document.getElementById(`analisisResumen_${sourceIndex}`).innerHTML;
  ...
  `;
        ...
        /*window.handleSampleSubmit = function (event) {
            ...
            // === 1. Validar campos fijos obligatorios ===
            const camposFijos = [
                { id: 'fechaEnvio', nombre: 'Fecha de envío' },
                { id: 'horaEnvio', nombre: 'Hora de envío' },
                { id: 'laboratorio', nombre: 'Laboratorio' },
                { id: 'empresa_transporte', nombre: 'Empresa de transporte' },
                { id: 'autorizado_por', nombre: 'Autorizado por' },
                { id: 'usuario_responsable', nombre: 'Usuario responsable' },
                { id: 'numeroSolicitudes', nombre: 'Número de solicitudes' }
            ];
            ...
            // === 2. Validar cada fila dinámica ===
            for (let i = 0; i < totalSamples; i++) {
                const prefijo = `Fila ${i + 1}:`;

                // a) Tipo de muestra
                const tipoMuestra = document.getElementById(`tipoMuestra_${i}`);
                if (!tipoMuestra || !tipoMuestra.value) {
                    errores.push(`${prefijo} Seleccione un tipo de muestra.`);
                }

                // b) Fecha de toma
                const fechaToma = document.getElementById(`fechaToma_${i}`);
                if (!fechaToma || !fechaToma.value) {
                    errores.push(`${prefijo} Ingrese la fecha de toma.`);
                }

                // c) Código de referencia (debe estar completo → campo oculto no vacío)
                const codRef = document.getElementById(`codigoReferenciaValue_${i}`);
                if (!codRef || !codRef.value.trim()) {
                    errores.push(`${prefijo} Complete el código de referencia.`);
                }

                // d) Al menos un análisis seleccionado
                const analisis = sampleDataCache[i]?.analisisSeleccionados || [];
                if (analisis.length === 0) {
                    errores.push(`${prefijo} Seleccione al menos un análisis.`);
                }
            }

            // === 3. Si hay errores, mostrar alerta y detener ===
            if (errores.length > 0) {
                alert("❌ Corrija los siguientes errores:\n\n" + errores.join('\n'));
                return;
            }

            // === 4. Si todo está OK, proceder ===
            const formData = new FormData(document.getElementById("sampleForm"));
  ...
  `;
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Delegar eventos en el documento
            document.body.addEventListener('mousedown', function (e) {
                if (e.target.tagName === 'SELECT' &&
                    (e.target.classList.contains('tipo-muestra-select') ||
                        e.target.classList.contains('numero-muestras-select'))) {
                    e.target.focus();
                }
            });
        });
