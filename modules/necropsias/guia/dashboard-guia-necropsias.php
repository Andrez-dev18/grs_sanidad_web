...
$rutaMd = __DIR__ . '/GUIA-NECROPSIAS.md';
$contenido = file_exists($rutaMd) ? file_get_contents($rutaMd) : 'Guía no encontrada.';

// URL del flipbook Heyzine para vista revista. Dejar vacío para mostrar mensaje "en preparación".
$urlRevistaNecropsias = '';
...
function md2html($md, &$toc) {
    ...
    foreach ($lines as $line) {
        ...
        if (preg_match('/^# (.+)$/', $trimmed, $m)) {
            ...
            $tit = $m[1];
            $secIcons = [
                'conceptos-del-modulo' => 'fa-lightbulb', 'flujo-de-uso' => 'fa-project-diagram',
                'registro' => 'fa-feather-pointed', 'listado' => 'fa-list', 'reporte-comparativo' => 'fa-chart-bar'
            ];
    ...
    $icons = [
        ...
        '[i:images]' => '<i class="fa-solid fa-images text-red-600" title="PDF Imágenes"></i>',
        ...
        '[i:excel]' => '<i class="fa-solid fa-file-excel text-emerald-600" title="Excel"></i>',
    ...
    $btns = [
        ...
        '[btn:guardar-necropsia]' => '<span class="guia-btn guia-btn-primary"><i class="fas fa-save"></i> Guardar Necropsia</span>',
        '[btn:filtrar]' => '<span class="guia-btn guia-btn-filtrar"><i class="fas fa-filter"></i> Filtrar</span>',
        '[btn:generar-reporte]' => '<span class="guia-btn guia-btn-primary"><i class="fas fa-file-pdf"></i> Generar Reporte</span>',
        '[btn:limpiar]' => '<span class="guia-btn guia-btn-limpiar">Limpiar</span>',
    ];
    foreach ($btns as $k => $v) $html = str_replace($k, $v, $html);
    $links = [
        'registro' => ['modules/necropsias/dashboard-necropsias-registro.php', '📋 Registro de Necropsias', 'Registro de necropsias', '5.1 Registro de Necropsias'],
        'listado' => ['modules/necropsias/dashboard-necropsias-listado.php', '📄 Listado de Necropsias', 'Listado de necropsias', '5.2 Listado de Necropsias'],
        'reporte' => ['modules/necropsias/dashboard-reporte-comparativo.php', '📊 Reporte Comparativo', 'Compare resultados entre galpones', '5.3 Reporte Comparativo'],
    ];
...
<body>
    <div class="guia-layout">
        ...
        <div class="guia-wrap">
            <?= $html ?>
            <p class="guia-footer"><i class="fas fa-info-circle"></i> Guía del Sistema de Sanidad — Módulo Necropsias</p>
        </div>
    </div>
    <script>
    (function() {
