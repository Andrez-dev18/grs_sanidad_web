...
<body class="bg-gray-50">
    ...
    (function() {
        ...
        if ($t.length && !$t.hasClass('dataTable')) {
            window.tableTipoProgramaDt = $t.DataTable({
                pageLength: 20,
                ...
                drawCallback: function() {
                    if (typeof window.renderizarTarjetasTipoProg === 'function') window.renderizarTarjetasTipoProg();
                },
                initComplete: function() {
                    var wrapper = $t.closest('.dataTables_wrapper');
                    var $controls = jQuery('#tipoProgramaDtControls');
