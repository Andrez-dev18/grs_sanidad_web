...
$rutaMd = __DIR__ . '/GUIA-TRACKING.md';
...
function md2html($md, &$toc) {
    ...
    foreach ($lines as $line) {
        ...
        if (preg_match('/^# (.+)$/', $trimmed, $m)) {
            ...
            $id = slug($m[1]); $tit = $m[1];
            $secIcons = [
                'conceptos-del-modulo' => 'fa-lightbulb', 'escaneo' => 'fa-qrcode',
                'seguimiento' => 'fa-location-dot', 'pendientes' => 'fa-clipboard-list'
            ];
        ...
        } elseif (preg_match('/^\[imagenes:([\d,]+)\]$/', $trimmed, $m)) {
            $closeBlock(); if ($inList) { $out[] = '</ol>'; $inList = false; }
            $out[] = '<!--IMAGENES:' . $m[1] . '-->';
        } elseif (preg_match('/^\[imagen-small:(\d+)\]$/', $trimmed, $m)) {
            $closeBlock(); if ($inList) { $out[] = '</ol>'; $inList = false; }
            $out[] = '<!--IMAGEN-SMALL:' . $m[1] . '-->';
    ...
    $icons = [
        ...
        '[i:eye]' => '<i class="fas fa-eye text-blue-600" title="Ver evidencia"></i>',
        ...
        '[i:list]' => '<i class="fas fa-list text-slate-600" title="Lista"></i>',
        '[i:th]' => '<i class="fas fa-th text-slate-600" title="Iconos"></i>',
    ];
    ...
    $btns = [
        ...
        '[btn:escanear]' => '<span class="guia-btn guia-btn-primary"><i class="fa-solid fa-qrcode"></i> Escanear / Recepcionar</span>',
    ];
    foreach ($btns as $k => $v) $html = str_replace($k, $v, $html);
    $links = [
        'escaneo' => ['modules/tracking/escaneo/dashboard-escaneoQR.php', 'Escaneo QR', 'Registrar recepción', '6.1 Escaneo QR'],
        'seguimiento' => ['modules/tracking/seguimiento_envios/dashboard-tracking-muestra.php', 'Seguimiento de envíos', 'Recepciones por código', '6.2 Seguimiento de envíos'],
        'pendientes' => ['modules/tracking/reporte/dashboard-reporte-tracking.php', 'Pendientes de entregas', 'Todos los registros y pendientes', '6.3 Pendientes de entregas'],
        'listado' => ['modules/reportes/dashboard-reportes.php', 'Listado de Muestras', 'Obtener código QR del envío', '2.2 Listado de Muestras'],
    ];
    ...
    if (is_dir($imgDir)) {
        $html = preg_replace_callback('/<!--IMAGENES:([\d,]+)-->/', function($m) use ($imgDir) {
            $nums = array_map('intval', explode(',', trim($m[1])));
            $figs = [];
            foreach ($nums as $n) {
                $base = $imgDir . 'imagen' . $n;
                $ext = file_exists($base . '.png') ? 'png' : (file_exists($base . '.jpg') ? 'jpg' : null);
                if ($ext) $figs[] = '<figure class="guia-imagen"><img src="imagenes/imagen' . $n . '.' . $ext . '" alt="Imagen ' . $n . '" loading="lazy"></figure>';
            }
            if (empty($figs)) return '';
            return '<div class="guia-imagenes-grid">' . implode('', $figs) . '</div>';
        }, $html);
        $html = preg_replace_callback('/<!--IMAGEN-SMALL:(\d+)-->/', function($m) use ($imgDir) {
            $n = (int)$m[1];
            $base = $imgDir . 'imagen' . $n;
            $ext = file_exists($base . '.png') ? 'png' : (file_exists($base . '.jpg') ? 'jpg' : null);
            if (!$ext) return '';
            return '<figure class="guia-imagen guia-imagen-small"><img src="imagenes/imagen' . $n . '.' . $ext . '" alt="Imagen ' . $n . '" loading="lazy"></figure>';
        }, $html);
        $html = preg_replace_callback('/<!--IMAGEN:(\d+)-->/', function($m) use ($imgDir) {
            ...
            $ext = file_exists($base . '.png') ? 'png' : (file_exists($base . '.jpg') ? 'jpg' : null);
            if (!$ext) return '';
            return '<figure class="guia-imagen guia-imagen-single"><img src="imagenes/imagen' . $n . '.' . $ext . '" alt="Imagen ' . $n . '" loading="lazy"></figure>';
        }, $html);
    } else {
        $html = preg_replace('/<!--IMAGENES:[\d,]+-->/', '', $html);
        $html = preg_replace('/<!--IMAGEN-SMALL:\d+-->/', '', $html);
...
<head>
    ...
    <style>
        ...
        .guia-imagenes-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1rem; justify-items: center; align-items: center; margin: 1rem 0; }
        .guia-imagenes-grid .guia-imagen { margin: 0; text-align: center; display: flex; align-items: center; justify-content: center; }
        .guia-imagenes-grid .guia-imagen img { max-width: 100%; max-height: 320px; object-fit: contain; }
        .guia-imagen-single { text-align: center; }
        .guia-imagen-small { text-align: center; margin: 0.5rem auto; max-width: 560px; }
        .guia-imagen-small img { max-width: 100%; max-height: 280px; object-fit: contain; }
...
<body>
    <div class="guia-layout">
        ...
        <nav class="guia-nav" id="guiaNav">
            ...
            <?php
            $navIcons = ['conceptos-del-modulo' => 'fa-lightbulb', 'flujo-de-uso' => 'fa-project-diagram', 'escaneo' => 'fa-qrcode', 'seguimiento' => 'fa-location-dot', 'pendientes' => 'fa-clipboard-list'];
            foreach ($toc as $item):
        ...
        <div class="guia-wrap">
            <?= $html ?>
            <p class="guia-footer"><i class="fas fa-info-circle"></i> Guía del Sistema de Sanidad — Módulo Tracking</p>
        </div>
    </div>
    <script>
    (function() {
