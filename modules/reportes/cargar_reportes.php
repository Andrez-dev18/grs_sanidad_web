    ...
    <?php while ($row = mysqli_fetch_assoc($result)): ?>
        ...
        <div class="report-card bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow mb-6 relative"
            ...
            <div class="absolute top-4 right-4 flex flex-col gap-2 w-36">
                ...
                <button
                    class="btn-enviar-correo flex items-center gap-2 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded-md shadow transition"
                    data-codigo="<?= htmlspecialchars($row['codEnvio']) ?>" title="Enviar por correo">
                    <span class="text-base min-w-[1.2em] text-center">✉️</span>
                    <span>Correo</span>
                </button>
                <button onclick="window.open('generar_qr_etiqueta.php?codigo=<?= urlencode($row['codEnvio']) ?>', '_blank')"
                    class="flex items-center gap-2 px-3 py-2 bg-green-400 hover:bg-green-500 text-white text-xs rounded-md shadow transition"
