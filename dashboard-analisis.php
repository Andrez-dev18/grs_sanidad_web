...
<body class="bg-gray-50">
    <div class="container mx-auto px-6 py-12">
                    ...
                    <table class="data-table w-full">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-sm font-semibold text-gray-800">Nombre del Análisis</th>
                                <th class="px-6 py-4 text-left text-sm font-semibold text-gray-800">Tipo de Muestra</th>
                                <th class="px-6 py-4 text-left text-sm font-semibold text-gray-800">Paquete de Análisis</th>
                        ...
                        <tbody id="analisisTableBody" class="divide-y divide-gray-200">
                            <?php
                            $query = "SELECT a.codigo, a.nombre, a.tipoMuestra, a.PaqueteAnalisis,
                            ...
                            if ($result && mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo '<td class="px-6 py-4 text-gray-600">' . htmlspecialchars($row['paquete_nombre'] ?? 'Ninguno') . '</td>';
                                    echo '<td class="px-6 py-4 flex gap-2">
                                        <button class="btn-icon p-2 text-lg hover:bg-blue-100 rounded-lg transition" 
                                                title="Editar" 
                                                onclick="openAnalisisModal(\'edit\', ' . (int)$row['codigo'] . ', \'' . 
                                                addslashes(htmlspecialchars($row['nombre'])) . '\', ' . 
                                                (int)$row['tipoMuestra'] . ', ' . 
                                                ($row['PaqueteAnalisis'] ? (int)$row['PaqueteAnalisis'] : 'null') . ')">
                                            ✏️
                                        </button>
                                ...
                                echo '<tr>';
                                echo '<td colspan="5" class="px-6 py-8 text-center text-gray-500">No hay análisis registrados</td>';
                                echo '</tr>';
                            }
                            ?>
                        </tbody>
        ...
        <div id="analisisModal" style="display: none;" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-2xl shadow-lg w-full max-w-lg">
                ...
                <div class="flex items-center justify-between p-6 border-b border-gray-200">
                    ...
                    <button onclick="closeAnalisisModal()" class="text-gray-500 hover:text-gray-700 text-2xl leading-none transition">
                        ×
                    </button>
                </div>
                    ...
                    <form id="analisisForm" onsubmit="return saveAnalisis(event)">
                        ...
                        <div class="form-field mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Nombre del Análisis <span class="text-red-500">*</span>
                            </label>
                            <input 
                                type="text" 
                                id="analisisModalNombre" 
                                name="nombre" 
                                maxlength="255" 
                        ...
                        <!-- Campo Paquete de Análisis -->
                        <div class="form-field mb-6">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Paquete de Análisis <span class="text-gray-500 text-xs">(Opcional)</span>
                            </label>
                            <select 
                                id="analisisModalPaquete" 
                                name="paqueteAnalisis"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition"
                            >
                                <option value="">Ninguno</option>
                                <?php
                                $query_paquetes = "SELECT codigo, nombre FROM com_paquetes_analisis ORDER BY nombre";
                                $result_paquetes = mysqli_query($conexion, $query_paquetes);
                                if ($result_paquetes) {
                                    while ($row = mysqli_fetch_assoc($result_paquetes)) {
                                        echo '<option value="' . $row['codigo'] . '">' . htmlspecialchars($row['nombre']) . '</option>';
                                    }
                                }
                                ?>
                            </select>
                            <p class="text-xs text-gray-500 mt-1">Asocie este análisis a un paquete existente</p>
                        </div>
