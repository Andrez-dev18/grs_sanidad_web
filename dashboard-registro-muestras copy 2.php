...
`;
                ...
                if (cache) {
                    ...
                    if (cache.analisisSeleccionados && cache.analisisSeleccionados.length > 0) {
                        // Fallback: si por error no hay HTML guardado (datos antiguos), generar uno simple
                        if (!cache.analisisResumenHtml) {
                            const nombres = cache.analisisSeleccionados
                                .map(a => (typeof a === 'string' ? `Análisis ${a}` : a.nombre))
                                .join(', ');
                            cache.analisisResumenHtml = `<small>${nombres}</small>`;
                        }
                        updateAnalisisResumen(i); // ← esto usará el HTML guardado
                    }
                }
            }
        }
        ...
        function updateAnalisisResumen(sampleIndex) {
            ...
            // ✅ Si tenemos el HTML bonito guardado, usamos eso
            if (cache.analisisResumenHtml) {
                resumenEl.innerHTML = cache.analisisResumenHtml;
                return;
            }

            // ✅ Si NO tenemos HTML bonito, pero sí tenemos objetos con nombre, generamos un resumen simple
            ...
            // ✅ Último recurso: si solo tenemos códigos (strings), usamos placeholder
  ...
  `;
                ...
                document.getElementById("analisisModalSaveBtn").onclick = () => {
                    ...
                    // ✅ Guardar OBJETOS COMPLETOS (código + nombre)
                    sampleDataCache[sampleIndex] = sampleDataCache[sampleIndex] || {};
                    sampleDataCache[sampleIndex].analisisSeleccionados = selectedAnalisis; // [{ codigo, nombre }, ...]

                    // ✅ Generar y guardar el HTML bonito
